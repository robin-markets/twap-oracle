...
import { PRICE_SCALE, type PolymarketMarketInfo, type TwapData } from '../types.js';
...
export async function verifyTwapDataBatch(items: VerificationInput[], polymarket: IPolymarketDataSource, config: VerificationConfig): Promise<void> {
    ...
    try {
        ...
        await sendNotification(
            `[WARN] Polymarket verification unavailable: batch fetch failed. ` + `Error: ${err instanceof Error ? err.message : String(err)}`,
        ).catch(() => {});
        return;
    }
    ...
    for (const item of toVerify) {
        ...
        if (!info) {
            await sendNotification(`[WARN] Market ${conditionId} not found on Polymarket during verification`).catch(() => {});
            continue;
        }
        ...
        if (subgraphResolved && polyResolved && info.resolvedYesPrice !== undefined) {
            const polyPrice = BigInt(Math.round(info.resolvedYesPrice * Number(PRICE_SCALE)));
        ...
        try {
            ...
            const twapResult = await polymarket.getTwapData(info.yesTokenId, Number(item.startTimestamp), Number(clampedEnd));
            ...
            if (subgraphTwap > 0 && polyTwap > 0) {
                const pctDiff = (Math.abs(subgraphTwap - polyTwap) / Math.max(subgraphTwap, polyTwap)) * 100;
                if (pctDiff > config.twapDivergenceThresholdPct) {
                    await sendNotification(
                        `[WARN] TWAP divergence for ${conditionId}: ` +
                            ...
                            `diff=${pctDiff.toFixed(1)}% (threshold: ${config.twapDivergenceThresholdPct}%)`,
                    ).catch(() => {});
                }
            }
        } catch (err) {
            await sendNotification(
                `[WARN] Polymarket TWAP comparison failed for ${conditionId}: ` + `${err instanceof Error ? err.message : String(err)}`,
            ).catch(() => {});
        }
    }
}
